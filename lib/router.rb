...
class Router
  ...
  # for syntactic sugar :)
  def draw(&proc)
    instance_eval(&proc)  #evaluate the proc in the context of this instance of Router
  end
  ...
  def run(req, res)
    matchedRoute = match(req)

    if matchedRoute.nil?
      res.status = 404
      res.write("No matched route.")
    else
      matchedRoute.run(req, res)
    end
